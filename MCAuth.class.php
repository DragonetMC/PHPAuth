 ...
 *
 *
 * @description Enhanced by DragonetMC Team.
 * @description Added ability to refresh/validate tokens. 
 * @author DefinitlyEvil
 */
class MCAuth {

    const SERVICE_URL   = 'https://authserver.mojang.com';
    ...
    const USER_AGENT    = 'Minecraft Launcher'; // User Agent used for requests
    ...
    public function validate($accessToken){
        ...
        $json['clientToken'] = $this->clientToken;
        $this->curl_json(self::SERVICE_URL . "/refresh", $json));
    ...
    public function refresh($accessToken){
        ...
        $json['accessToken'] = $accessToken;
        $json['clientToken'] = $this->clientToken;
        if ($this->curl_json(self::SERVICE_URL . "/refresh", $json)) {
            if(isset($this->curlresp->accessToken)){
                return $this->curlresp->accessToken;
            }else{
                return FALSE;
            }
        }else{
